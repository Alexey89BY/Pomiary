...
class PointRangeGraph(context: Context?) : View(context) {
    ...
    public override fun onDraw(canvas: Canvas) {
        ...
        val scale = (xpr - xpl) / (1.2F * 2.0F * (pointTolerance.offset + DataStorage.getToleranceNok()).toFloat()) // zoom * offset+-
        val x0 = (xpr + xpl) * 0.5F
        val y0 = 1.5F * dyt
        ...
        val pointZero = pointTolerance.origin
        val ptl = pointTolerance.origin - pointTolerance.offset
        val ptr = pointTolerance.origin + pointTolerance.offset
